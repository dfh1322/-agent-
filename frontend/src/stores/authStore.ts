...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  login: async (username: string, password: string) => {
    ...
    try {
      ...
      const tokenAfter = typeof localStorage !== 'undefined'
        ...
        : null;
      const userAfter: AuthUser | null = authApi.getCurrentUser() as unknown as AuthUser | null;
      set({
        user: (userAfter ?? auth.user) as unknown as AuthUser,
  ...
  register: async (data: any) => {
    const auth = await authApi.register(data) as unknown as { user: AuthUser };
    authApi.setAuth(auth as any);
    set({ user: (auth as any).user ?? null, isAuthenticated: true });
  },
