...
const LandlordContactDrawer: React.FC<LandlordContactDrawerProps> = ({
  ...
  useEffect(() => {
    if (visible && community?.owner_id) {
      setLoading(true);
      setContact(null);
      setSubmitted(false);
      form.resetFields();
      landlordApi
        .getContactInfo(community.owner_id)
        .then((res) => {
  ...
  }, [visible, community, form, message]);
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    // Always include community_id; landlord_id is optional
    // (backend resolves landlord_id from community if not provided)
    if (!community) return;
    setSubmitting(true);
    try {
      ...
      if (community.owner_id) {
        payload.landlord_id = community.owner_id;
      }
      const res = await landlordApi.submitContactMessage(payload);
      if (res.success) {
        setSubmitted(true);
