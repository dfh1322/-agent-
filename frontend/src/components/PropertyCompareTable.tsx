/**
 * CommunityCompareTable - 多小区动态列对比表格
 *
 * 功能：
 *   - 动态列：根据传入的小区数据自动生成对比列
 *   - 高亮差异项：不同值自动标色
...
interface CommunityCompareTableProps {
  /** 小区数据列表 */
...
const CommunityCompareTable: React.FC<CommunityCompareTableProps> = ({
  communities,
  columns,
  aiRecommendation,
  ...
  const diffColumns = useMemo(() => {
    ...
    columns.forEach((col) => {
      const values = communities.map((p) => String(p[col.dataIndex]));
      if (new Set(values).size > 1) {
        diffSet.add(col.dataIndex);
      }
    });
    return diffSet;
  }, [communities, columns, highlightDifferences]);
  ...
  const tableColumns = useMemo(() => {
    ...
    const communityCols = communities.map((prop) => ({
      title: (
      ...
      render: (text: any, record: any, index: number) => {
        ...
        if (colDef.numeric && stat && !isNaN(parseFloat(String(value)))) {
          ...
          if (isBetter && communities.length > 1) {
            icon = <CheckCircleOutlined style={{ color: '#52c41a', marginLeft: 4 }} />;
          } else if (isWorse && communities.length > 1) {
    ...
    return [nameCol, ...communityCols];
  }, [communities, columns, diffColumns, columnStats]);
