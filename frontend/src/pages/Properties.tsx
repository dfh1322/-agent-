...
const Properties: React.FC = () => {
  ...
  const loadFavorites = async () => {
    try {
      ...
      if (res.success) {
        ...
        res.data.forEach((f: any) => { map[f.property_id] = true; });
        setFavoritedMap(map);
      }
    } catch { /* ignore */ }
  };
  ...
  const handleRecommend = async () => {
    ...
    try {
      ...
      if (result.success) { setRecommendation(result.recommendation); message.success('AI推荐完成'); }
      else message.error(result.error || '推荐失败，请稍后重试');
  ...
  return (
    <Layout style={{ minHeight: '100dvh', background: 'var(--color-bg)' }}>
      ...
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <Text style={{ fontSize: 16, fontWeight: 500, color: 'var(--color-ink)' }}>
            找到 <Text strong style={{ color: '#3b82f6' }}>{filteredProperties.length}</Text> 个符合条件的房源
          </Text>
        </div>
      ...
      <LandlordContactDrawer
        ...
        onClose={() => { setDrawerOpen(false); setSelectedProperty(null); }}
      />
    </Layout>
  );
};
