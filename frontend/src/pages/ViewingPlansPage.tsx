...
export default function ViewingPlansPage() {
  ...
  const loadPlans = async () => {
    try {
      ...
      if ((res as any).success) setPlans((res as any).data);
  ...
  const handleCreate = async () => {
    ...
    const ids = communityIds.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n));
    if (ids.length === 0) { alert('请输入至少一个楼盘ID（用逗号分隔）'); return; }
    try {
      await viewingPlanApi.create({ title, community_ids: ids, plan_date: planDate || undefined, notes: notes || undefined });
      setShowForm(false); setTitle(''); setCommunityIds(''); setPlanDate(''); setNotes('');
      loadPlans();
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px' }}>
        ...
        {showForm && (
          <div style={{
            ...
            <input style={inputStyle} placeholder="楼盘ID（用逗号分隔，如：1,3,5）" value={communityIds} onChange={e => setCommunityIds(e.target.value)} />
