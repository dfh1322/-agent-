...
import { Card, Row, Col, Table, Tag, Typography, Spin, Tooltip, Space } from 'antd';
import {
        ...
        return (
          <div key={d.label} role="group" aria-label={`${d.label} ${d.value}`}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <Text strong style={{ color: palette.ink, ...text.number }}>
                {d.value}{unit}
              </Text>
            </div>
            <div
...
const AdminStatistics: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled && (res as any)?.success) setStats((res as any).data as StatData);
      } catch {
  ...
  const hotColumns: any[] = [
      ...
      render: (_: unknown, __: unknown, i: number) => (
        <Text
          style={{
            ...
            fontWeight: i < 3 ? text.subtitle.fontWeight : text.body.fontWeight,
          }}
        >
          {i + 1}
        </Text>
    ...
    },
    {
      ...
      render: (_id: number, record: { district_name?: string }) => (
        <Tag color={palette.primary}>{record.district_name || `ID ${_id}`}</Tag>
      ),
    },
    {
      title: '综合热度', key: 'heat_score', width: 90,
      render: (_: unknown, r: { heat_score?: number }) => (
        <span>
          <RiseOutlined style={{ color: palette.success, marginRight: 6 }} />
          <Text style={{ ...text.number, fontWeight: text.subtitle.fontWeight }}>
            {r.heat_score != null ? (r.heat_score * 100).toFixed(0) : '—'}
          </Text>
        </span>
      ),
    },
    {
      title: '来源', key: 'sources', width: 160,
      render: (_: unknown, r: { favs?: number; chat_mentions?: number; tool_hits?: number }) => (
        <Space size={4} wrap>
          {r.tool_hits ? (
            <Tooltip title="工具调用命中">
              <Tag style={{ background: palette.accentLight, color: palette.accent, border: 'none' }}>
                <ToolOutlined style={{ marginRight: 2 }} />{r.tool_hits}
              </Tag>
            </Tooltip>
          ) : null}
          {r.chat_mentions ? (
            <Tooltip title="对话回复提及">
              <Tag style={{ background: palette.primaryLight, color: palette.primary, border: 'none' }}>
                <CommentOutlined style={{ marginRight: 2 }} />{r.chat_mentions}
              </Tag>
            </Tooltip>
          ) : null}
          {r.favs ? (
            <Tooltip title="用户收藏">
              <Tag style={{ background: palette.warningLight, color: palette.ink, border: 'none' }}>
                <StarOutlined style={{ marginRight: 2 }} />{r.favs}
              </Tag>
            </Tooltip>
          ) : null}
          {!r.tool_hits && !r.chat_mentions && !r.favs && <Text style={{ fontSize: 11, color: palette.inkMuted }}>—</Text>}
        </Space>
      ),
    },
  ];
            ...
            >
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                ...
                <StatRow
                  ...
                  label="最近 24h 对话"
                  value={String(stats?.conversations?.last_24h ?? 0)}
