...
export default function FavoritesPage() {
  ...
  const handleRemove = async (fav: FavoriteItem) => {
    if (!window.confirm(`确定要取消收藏「${fav.property_name}」吗？`)) return;
    try {
      await favoriteApi.remove(fav.property_id);
          ...
          favorites.map(fav => (
            <div key={fav.id} className="glass-card" style={{
              ...
              <div style={{ cursor: 'pointer', flex: 1 }} onClick={() => navigate('/properties')}>
                <h4 style={{ margin: '0 0 8px', fontSize: 18, color: 'var(--color-ink)' }}>{fav.property_name}</h4>
