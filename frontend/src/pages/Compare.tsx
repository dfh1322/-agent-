...
import { Navbar, CommunityCompareTable } from '../components';
import type { Community } from '../types/property';
...
const Compare: React.FC = () => {
  ...
  const loadCommunities = async () => {
    try {
      ...
      if (Array.isArray(result)) {
        setAllCommunities(result as any[]);
        if (result.length >= 3) setSelectedIds((result as any[]).slice(0, 3).map(p => p.id!));
      }
    } catch (error) { console.error('加载楼盘失败:', error); }
  };

  const selectedCommunities = allCommunities.filter(p => selectedIds.includes(p.id!));
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      if ((result as any).success) { setAnalysis((result as any).analysis); message.success('AI分析完成'); }
      else message.error((result as any).error || '分析失败，请稍后重试');
  ...
  const compareData = selectedCommunities.map(prop => ({
    key: prop.id,
    name: prop.name,
      ...
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: '24px' }}>
        ...
        <Card style={sectionCard}>
          ...
          <Space wrap style={{ width: '100%' }}>
            <Select mode="multiple" value={selectedIds} onChange={setSelectedIds}
              ...
              options={allCommunities.map(p => ({ label: `${p.name} (${p.district})`, value: p.id! }))} />
        ...
        {selectedCommunities.length >= 2 && (
          <Card style={sectionCard}>
            ...
            <CommunityCompareTable communities={compareData} columns={compareColumns} aiRecommendation={analysis} />
          </Card>
        )}
