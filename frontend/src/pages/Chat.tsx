...
async function fetchDbSuggestions(): Promise<DbSuggestion[]> {
  ...
  try {
    ...
    /* 2. 热门楼盘（从在售列表中取前几个） */
    const propsRes: any = await chatApi.getCommunities();
...
const Chat: React.FC = () => {
  ...
  /* ── 预填楼盘分析 ── */
  const communityContext = (location.state as { propertyName?: string; communityName?: string; propertyId?: number; communityId?: number } | null);
  useEffect(() => {
    const name = communityContext?.propertyName || communityContext?.communityName;
    if (name && !hasSentPrefilled.current) {
      hasSentPrefilled.current = true;
      setInput(`请帮我详细分析一下「${name}」这个楼盘`);
        ...
        if (latest && latest.backendId) {
          try {
            ...
            if (msgRes.success && Array.isArray(msgRes.data)) {
              const loaded: Message[] = msgRes.data
              ...
              // 仅在无历史消息时附加欢迎语
              if (loaded.length === 0 || loaded[0].id !== 'welcome') {
                loaded.unshift(WELCOME_MESSAGE);
              }
              setMessages(loaded);
              // 标记活跃会话
  ...
  const deleteSession = useCallback((sessionId: string) => {
    ...
    if (session?.backendId) {
      chatApi.deleteConversation(session.backendId).catch(() => { /* 静默失败 */ });
    }
      ...
      requestAnimationFrame(() => {
        ...
        (async () => {
          ...
          try {
            ...
            const response: any = await chatApi.sendMessage(apiMessages as any, sessions[0]?.id);
