...
const InputSelect: React.FC<{
  ...
  const filtered = options
    ...
    .filter((o) => !search || o.value.includes(search) || o.label.includes(search));
        ...
        {value.map((v) => (
          ...
          >
            ...
            <span
              ...
              style={{ cursor: 'pointer', fontSize: 16, lineHeight: 1, opacity: 0.6 }}
            >
              ×
            </span>
          </span>
      ...
      {open && (search || filtered.length > 0) && (
        ...
        >
          {filtered.slice(0, 30).map((o) => (
            ...
            >
              <span>{o.value}</span>
              <span style={{ fontSize: 12, color: 'var(--color-ink-muted)' }}>{o.label}</span>
            </div>
          ))}
          {search && !filtered.some((o) => o.value === search.trim()) && (
            <div
              onClick={addCustom}
              style={{
                padding: '8px 14px',
  ...
  const loadAll = async () => {
    try {
      ...
      if (pref.success && pref.data) {
        ...
        setPreferredDistricts(pref.data.preferred_districts || []);
        setDistrictNames(pref.data.preferred_districts || []);
    ...
    try {
      ...
      if (role === 'landlord') {
        const res: any = await landlordApi.getMyCommunities();
  ...
  const handleSavePrefs = async () => {
    try {
      await settingsApi.updatePreference({
        ...
        preferred_districts: preferredDistricts.length ? preferredDistricts : undefined,
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 20px 48px' }}>
        ...
        {role === 'user' && (
          <div style={sectionCard}>
            ...
            {/* Preferred districts */}
            <div style={{ marginBottom: 20 }}>
              <label style={{ ...labelCss, fontSize: 13, fontWeight: 600, color: 'var(--color-ink)' }}>意向区域</label>
              <InputSelect
                value={preferredDistricts}
                onChange={(values) => {
                  if (Array.isArray(values)) {
                    setPreferredDistricts(values as string[]);
                  }
                }}
                placeholder="开始输入区域名称，如：西湖区、朝阳区..."
                options={[
                  // 热门城市区域快捷选项
                  { label: '🏙️ 北京', value: '朝阳区' },
                  { label: '🏙️ 北京', value: '海淀区' },
                  { label: '🏙️ 北京', value: '西城区' },
                  { label: '🏙️ 北京', value: '东城区' },
                  { label: '🏙️ 上海', value: '浦东新区' },
                  { label: '🏙️ 上海', value: '徐汇区' },
                  { label: '🏙️ 上海', value: '黄浦区' },
                  { label: '🏙️ 上海', value: '静安区' },
                  { label: '🏙️ 广州', value: '天河区' },
                  { label: '🏙️ 广州', value: '海珠区' },
                  { label: '🏙️ 广州', value: '越秀区' },
                  { label: '🏙️ 深圳', value: '南山' },
                  { label: '🏙️ 深圳', value: '福田' },
                  { label: '🏙️ 深圳', value: '宝安' },
                  { label: '🏙️ 杭州', value: '西湖区' },
                  { label: '🏙️ 杭州', value: '滨江区' },
                  { label: '🏙️ 杭州', value: '上城区' },
                  { label: '🏙️ 杭州', value: '拱墅区' },
                  { label: '🏙️ 杭州', value: '萧山区' },
                  { label: '🏙️ 杭州', value: '余杭区' },
                  { label: '🏙️ 成都', value: '高新区' },
                  { label: '🏙️ 成都', value: '锦江区' },
                  { label: '🏙️ 成都', value: '武侯区' },
                  { label: '🏙️ 武汉', value: '武昌区' },
                  { label: '🏙️ 武汉', value: '洪山区' },
                  { label: '🏙️ 武汉', value: '江岸区' },
                  { label: '🏙️ 南京', value: '鼓楼区' },
                  { label: '🏙️ 南京', value: '建邺区' },
                  { label: '🏙️ 南京', value: '江宁区' },
                  { label: '🏙️ 重庆', value: '渝中区' },
                  { label: '🏙️ 重庆', value: '江北区' },
                  { label: '🏙️ 天津', value: '和平区' },
                  { label: '🏙️ 苏州', value: '工业园区' },
                  { label: '🏙️ 苏州', value: '姑苏' },
                  { label: '🏙️ 西安', value: '雁塔区' },
                  { label: '🏙️ 西安', value: '高新区' },
                  { label: '🏙️ 长沙', value: '岳麓区' },
                  { label: '🏙️ 青岛', value: '市南' },
                  { label: '🏙️ 青岛', value: '崂山' },
                  { label: '🏙️ 厦门', value: '思明' },
                  { label: '🏙️ 郑州', value: '金水区' },
                  { label: '🏙️ 合肥', value: '蜀山区' },
                  { label: '🏙️ 济南', value: '历下区' },
                  { label: '🏙️ 沈阳', value: '和平区' },
                  { label: '🏙️ 昆明', value: '五华区' },
                  { label: '🏙️ 南昌', value: '红谷滩区' },
                  { label: '🏙️ 南宁', value: '青秀区' },
                  { label: '🏙️ 福州', value: '鼓楼区' },
                ]}
              />
            </div>
