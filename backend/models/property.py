...
对应 MySQL 表：
  districts, properties, house_types, property_images,
  property_facilities, property_risks, price_history,
...
class Property(Base, TimestampMixin):
    ...
    property_type = Column(String(50))  # 住宅/公寓/别墅
    ...
    decoration_status = Column(String(20))  # 毛坯/简装/精装
    school_district = Column(String(200))
    metro_distance = Column(Integer)  # 米
    metro_line = Column(String(50))
    status = Column(String(20), default="在售")  # 在售/待售/售罄
    tags = Column(JSON)
    description = Column(Text)
...
class HouseType(Base, TimestampMixin):
    ...
    property = relationship("Property", back_populates="house_types")
...
class PropertyImage(Base, TimestampMixin):
    ...
    image_type = Column(String(20))  # 效果图/实景图/样板间/区位图
...
class PropertyFacility(Base, TimestampMixin):
    __tablename__ = "property_facilities"

    id = Column(Integer, primary_key=True, index=True)
    property_id = Column(Integer, ForeignKey("properties.id"), nullable=False)
    facility_type = Column(String(20), nullable=False)  # 学校/医院/商超/地铁/公园
    name = Column(String(100), nullable=False)
...
class PropertyRisk(Base, TimestampMixin):
    __tablename__ = "property_risks"

    id = Column(Integer, primary_key=True, index=True)
    property_id = Column(Integer, ForeignKey("properties.id"), nullable=False)
    risk_type = Column(String(20), nullable=False)  # 高架/垃圾站/高压线/其它
...
class Favorite(Base, TimestampMixin):
    ...
    # ORM 层失去了唯一性约束的双重保护。
    __table_args__ = (
        UniqueConstraint("user_id", "property_id", name="uk_user_property"),
    )
    ...
    property_ref = relationship("Property", back_populates="favorite_list")
