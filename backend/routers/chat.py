...
def extract_requirements(text: str) -> dict:
    requirements = {
        ...
        "metro": False, "school": False, "floor_preference": None,
    }
    ...
    if pm:
        ...
        if "左右" in text or "大概" in text or "上下" in text:
            requirements["min_price"] = max(price - 20, 0)
            requirements["max_price"] = price + 20
        else:
            # 单值预算 → 视为上限，min=50%, max=130% 容忍度
            requirements["min_price"] = price * 0.5
            requirements["max_price"] = price * 1.3
    ...
    # 楼层偏好
    if re.search(r"(高楼层|高层|顶楼|顶层|高区)", text):
        requirements["floor_preference"] = "高楼层"
    elif re.search(r"(低楼层|低层|底楼|底层|低区|一楼|二楼)", text):
        requirements["floor_preference"] = "低楼层"
    elif re.search(r"(中楼层|中层|中间楼层|中区)", text):
        requirements["floor_preference"] = "中楼层"
    elif re.search(r"(\d+)\s*层以上", text):
        requirements["floor_preference"] = text

    # 朝向
    if re.search(r"(朝南|南向|向南|南北通透|南北)", text):
        requirements["orientation"] = "南" if "北" not in text else "南北"
    elif re.search(r"(朝东|东向)", text):
        requirements["orientation"] = "东"
    elif re.search(r"(朝西|西向)", text):
        requirements["orientation"] = "西"

    # 楼栋/小区结构查询
    reqs_structured = {}
    if re.search(r"(几栋|多少栋|几幢|多少幢|几个单元|几层|多少层|总层|共.*层)", text):
        reqs_structured["ask_building_count"] = True
    if re.search(r"(哪个楼栋|几号楼|哪栋|哪幢|几幢|具体.*楼栋|楼栋名称)", text):
        reqs_structured["ask_specific_building"] = True
    requirements["_structured_query"] = reqs_structured
...
async def get_properties(
    ...
    普通 user 维持原行为（看全部在售）。
    """
    from models.property import Community, Building, HouseType, District
    from sqlalchemy import or_

    query = db.query(Community).filter(Community.status == "在售")
    if current_user.role == "landlord":
        query = query.filter(Community.owner_id == current_user.id)
    if district:
        query = query.join(District).filter(District.name.contains(district))
    if min_price:
        query = query.filter(Community.total_price_min >= min_price)
    if max_price:
        query = query.filter(Community.total_price_max <= max_price)
    if bedrooms:
        query = query.join(Building, Building.community_id == Community.id) \
...
async def get_property_detail(property_id: int, db: Session = Depends(get_db)):
    """获取楼盘/小区详情，优先走 community + building 层级。"""
    from models.property import Community, Building, HouseType
    ...
    if c:
        ...
        for b in buildings:
            ...
            bld_list.append({
                ...
                "units_per_floor": b.units_per_floor, "orientation": b.orientation,
                "status": b.status,
                "house_types": [{
        ...
        return {
            ...
            "metro_distance": c.metro_distance, "metro_line": c.metro_line,
...
async def compare_properties(request: dict, db: Session = Depends(get_db)):
    ids = request.get("community_ids") or []
    if not ids:
    ...
    set_db_context(db)
    result = tool_compare.invoke({"community_ids": ids})
    ...
    if items:
        ...
        analysis_lines = [f"为您对比了 {len(items)} 个楼盘：{'、'.join(names)}。\n"]
        analysis_lines.append("【综合对比分析】\n")

        # 价格对比
        prices = [(it["name"], it.get("total_price_min")) for it in items if it.get("total_price_min")]
        if prices:
            prices.sort(key=lambda x: x[1] or 0)
            analysis_lines.append(f"- 价格优势：{prices[0][0]} 起步价最低（{prices[0][1]}万），适合预算有限的选择。")

        # 面积对比
        areas = [(it["name"], it.get("area_min")) for it in items if it.get("area_min")]
        if areas:
            areas.sort(key=lambda x: x[1] or 0)
            analysis_lines.append(f"- 户型面积：{areas[-1][0]} 面积最大（{areas[-1][1]}㎡起），适合大户改善需求。")

        # 地铁
        metros = [(it["name"], it.get("metro_distance")) for it in items if it.get("metro_distance") is not None]
        near = [m for m in metros if (m[1] or 9999) <= 1500]
        if near:
            analysis_lines.append(f"- 地铁出行：{'、'.join([n for n, _ in near])} 距离地铁 1.5km 以内，通勤便利。")

        # 学区
        schools = [it["name"] for it in items if it.get("school_district")]
        if schools:
            analysis_lines.append(f"- 学区资源：{'、'.join(schools)} 带学区，适合有子女入学需求的家庭。")

        # 绿化
        greens = [(it["name"], it.get("green_rate")) for it in items if it.get("green_rate")]
        if greens:
            greens.sort(key=lambda x: x[1] or 0, reverse=True)
            analysis_lines.append(f"- 居住品质：{greens[0][0]} 绿化率最高（{greens[0][1]}%），环境更优。")

        analysis_lines.append(f"\n建议您根据自身需求（通勤、学区、预算、面积）综合权衡，选择最匹配的楼盘。")
        analysis = "\n".join(analysis_lines)

    return {"success": True, "analysis": analysis, "comparison": result, **result}


@router.post("/policy/explain")
async def explain_policy(request: dict, db: Session = Depends(get_db)):
    question = request.get("question", "")
    from tools.base import set_db_context, search_policy, web_search
    set_db_context(db)
    ...
    if items:
        ...
        # 数据库未命中 → 网络搜索
        web_res = web_search.invoke({"query": question, "top_k": 3})
        web_items = web_res.get("items") or []
        if web_items:
            notice = "数据库暂未学习到相关信息，以下为您网络查询的结果（仅供参考）：\n\n"
            explanation = notice + "\n\n".join(
                f"【{w.get('title', '网络来源')}】\n{w.get('snippet', '')[:600]}\n来源：{w.get('url', '')}"
                for w in web_items
            )
        else:
            explanation = "数据库和网络均未查询到相关信息，无法解答。请尝试更具体的关键词，如「公积金」「限购」「契税」。"
...
async def generate_personalized_plan(request: dict, db: Session = Depends(get_db)):
    """生成个性化置业方案。

    输入用户偏好（预算、区域、户型、首付等），自动串联三个工具：
        * search_properties → 匹配楼盘
        * calculate_mortgage → 计算月供
        * search_policy → 查询相关购房政策
    返回结构化方案 JSON。
    """
    from tools.base import set_db_context, search_properties, calculate_mortgage, search_policy

    set_db_context(db)

    district = str(request.get("district") or "")
    max_price = float(request.get("max_price") or request.get("budget") or 0)
    min_price = float(request.get("min_price") or 0)
    bedrooms = int(request.get("bedrooms") or 3)
    down_payment_ratio = float(request.get("down_payment_ratio") or 0.3)
    loan_term = int(request.get("loan_term") or 30)
    has_pf = bool(request.get("has_provident_fund", False))
    is_second = bool(request.get("is_second_home", False))
    need_metro = bool(request.get("need_metro", False))
    need_school = bool(request.get("need_school", False))

    # 1. 匹配楼盘
    search_args: dict = {"top_k": 5, "limit": 5}
    if district:
    ...
    if bedrooms > 0:
        search_args["bedrooms"] = bedrooms

    prop_result = search_properties.invoke(search_args)
    properties = prop_result.get("items") or []

    # 2. 对目标总价计算月供（取用户预算中位数或第一个楼盘均价）
    calc_price = max_price if max_price > 0 else (
        (float(properties[0].get("total_price_min", 300) or 300)
         + float(properties[0].get("total_price_max", 400) or 400)) / 2
        if properties else 300
    )
    mortgage = calculate_mortgage.invoke({
        "price": calc_price,
    ...
    policy_keywords = []
    if district:
        policy_keywords.append(district.replace("区", ""))
    policy_keywords.extend(["购房", "限购", "公积金", "首付"])
    policies_res = search_policy.invoke({"keyword": " ".join(policy_keywords), "top_k": 3})
    policies = policies_res.get("items") or []

    # 4. 拼装方案
    user_needs: list[str] = []
    if district:
        user_needs.append(f"意向区域：{district}")
    if max_price > 0:
        user_needs.append(f"预算上限：{max_price:.0f} 万元")
    if bedrooms > 0:
        user_needs.append(f"户型需求：{bedrooms} 室")
    if need_metro:
        user_needs.append("需要近地铁")
    if need_school:
        user_needs.append("需要考虑学区")
    ...
    for p in properties[:5]:
        entry = {
            ...
            "price_range": p.get("total_price_range"),
            "area_range": p.get("area_range"),
        }
        pros: list[str] = []
        cons: list[str] = []
        if p.get("metro_distance") is not None:
            d = float(p["metro_distance"])
            if d <= 800:
                pros.append(f"近地铁({d:.0f}m)")
            elif d > 2000:
                cons.append(f"距地铁较远({d:.0f}m)")
        if p.get("school_district"):
            pros.append("带学区")
        if p.get("green_rate"):
            g = float(p.get("green_rate", 0) if p["green_rate"] is not None else 0)
            if g >= 35:
                pros.append(f"高绿化率({g:.0f}%)")
        entry["pros"] = pros
        entry["cons"] = cons
        recommended.append(entry)

    # 预算建议
    down = mortgage.get("down_payment", 0)
    monthly = mortgage.get("monthly_payment", 0)
    total_interest = mortgage.get("total_interest", 0)
    budget_advice = (
        f"按 {calc_price:.0f} 万元总价、{down_payment_ratio*100:.0f}% 首付计算，"
        f"您需要准备首付约 {down:.1f} 万元，月供约 {monthly:.2f} 万元，"
        f"30 年总利息约 {total_interest:.1f} 万元。"
        f"建议月供不超过家庭月收入的 50%。"
    )
    ...
    tips: list[str] = []
    if recommended:
        tips.append(f"为您匹配了 {len(recommended)} 个{'带学区' if need_school else ''}楼盘，建议重点对比价格和通勤距离。")
    if policies:
        tips.append(f"相关购房政策已附在方案中，请仔细阅读「{policies[0].get('title', '购房政策')}」等文件。")
    tips.append("建议实地考察 2-3 个意向楼盘后再做决定。")
    overall_advice = "\n".join(f"• {t}" for t in tips)

    title = f"{district or '杭州'}·{'学区' if need_school else '品质'}置业方案"

    report = {
        "title": title,
        "generated_at": __import__("datetime").datetime.now().isoformat(),
        "userNeeds": "\n".join(f"• {n}" for n in user_needs) if user_needs else "基础购房需求",
        "recommendedProperties": recommended,
        "budgetAdvice": budget_advice,
        "overallAdvice": overall_advice,
        "policies": [
            {"title": p.get("title"), "content": (p.get("content") or "")[:300]}
            for p in policies
        ],
    }
