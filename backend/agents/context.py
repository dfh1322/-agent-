...
def rank_properties_by_context(
    ...
    def score(c: Community) -> int:
        s = 0
        if weather.get("is_rainy") and c.metro_distance and c.metro_distance <= 800:
            s += 3
        if weather.get("is_hot") and c.green_rate and float(c.green_rate) >= 35:
            s += 2
        if weather.get("is_cold") and c.decoration_status and "精装" in c.decoration_status:
            s += 1
        if c.is_featured:
            s += 1
        return s

    return sorted(communities, key=score, reverse=True)
