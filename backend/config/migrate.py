...
以幂等方式为 ``users`` 和 ``properties`` 表添加缺失列、创建缺失索引。
...
def run_migrations() -> None:
    ...
    if "properties" in table_names:
        ...
        prop_idx = {i["name"] for i in inspector.get_indexes("properties")}
        with engine.begin() as conn:
            if "owner_id" not in prop_cols:
                conn.execute(text("ALTER TABLE properties ADD COLUMN owner_id INT"))
            # 行政区划迁移（与 ``backend/models/property.py`` 的 ORM
            # 保持一致，供 Cascader 树形下拉使用）。
            if "province" not in prop_cols:
                conn.execute(
                    text("ALTER TABLE properties ADD COLUMN province VARCHAR(30) NULL")
                )
            if "city" not in prop_cols:
                conn.execute(
                    text("ALTER TABLE properties ADD COLUMN city VARCHAR(30) NULL")
                )
            if "district_name" not in prop_cols:
                conn.execute(
                    text(
            ...
            if "ix_properties_region" not in prop_idx:
                conn.execute(
                    text(
                        "CREATE INDEX ix_properties_region "
                        "ON properties (province, city, district_name)"
                    )
                )
    ...
    _TIMESTAMP_TABLES = [
        ...
        "price_history", "property_facilities", "property_images",
        "property_risks",
    ]
